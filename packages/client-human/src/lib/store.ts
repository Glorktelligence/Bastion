...
/**
 * Svelte-compatible store primitives that work in both browser and Node.js.
 * Implements the Svelte store contract ({ subscribe }) without importing svelte.
 */
...
export function writable<T>(initial: T): Writable<T> {
  ...
  return {
    subscribe(fn: (value: T) => void): () => void {
      subscribers.add(fn);
      fn(value); // Svelte contract: call immediately with current value
      return () => {
